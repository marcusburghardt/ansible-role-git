...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Script to optimize the PR review process in local environment. "
            "It must be called from the repository which the PR belongs to."
        ),
    ...
    parser.add_argument(
        "--main-branch", default="main", help="Main branch name in Upstream repository"
    )
    parser.add_argument(
        "--no-checkout",
        action="store_true",
...
def main():
    ...
    if fetch_pr_cmd.returncode != 0:
        ...
        if update_pr_cmd.returncode != 0:
            raise RuntimeError(
                "Failed to fetch or update the PR. Check the error messages."
            )
